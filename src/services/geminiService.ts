 ...
 * including business document ingestion & automated database importing.
 */

import { apiRequest } from './apiClient';
import { BillingItem, Client, ProductService, Invoice, Expense } from '../types';
import { parseCsvRows, ParsedDocument } from '../utils/fileParser';
    ...
    const data = await apiRequest<{ success: boolean; reply?: string; actions?: AgentAction[] }>('/api/ai/chat', {
      ...
      body: JSON.stringify({
        ...
        document: attachedDoc ? {
          ...
          content: attachedDoc.textContent.slice(0, 10000)
        } : undefined,
...
You assist the business owner with event management, equipment hire, quotations, invoicing, payments, expenses, client records, and document restructuring.
Always refer to the system as Binti Events Management System or Binti Events.
If a document or table is uploaded, extract its records into structured fields and prepare an import AgentAction with confirmation required.`
      })
    });
...
function extractActionsFromPrompt(prompt: string, context?: SaaSContext, attachedDoc?: ParsedDocument | null): AgentAction[] {
  ...
  if (attachedDoc && attachedDoc.textContent) {
    ...
    const isCsv = attachedDoc.fileType === 'csv' || attachedDoc.mimeType.includes('csv');

    if (isCsv || docText.includes('phone') || docText.includes('email') || docText.includes('company') || docText.includes('client')) {
...
/**
 * Deterministic local fallback with document restructuring support.
 */
function getLocalIntelligentFallback(prompt: string, context?: SaaSContext, attachedDoc?: ParsedDocument | null): AssistantResponse {
  ...
  // 1. If user uploaded a document (CSV, Client List, Catalog, Sales Data)
  if (attachedDoc && attachedDoc.textContent) {
    const isCsv = attachedDoc.fileType === 'csv' || attachedDoc.textContent.includes(',');
    const rows = parseCsvRows(attachedDoc.textContent);
    ...
    if (rows.length > 0) {
      ...
      reply += `I have parsed your file and identified **${parsedClients.length} structured records**:\n\n`;
      ...
      reply += `\n\nWould you like me to import these records directly into your **Binti Events** live database? Click the confirmation action below to execute.`;
    ...
    let reply = `### 📄 Document Uploaded: ${attachedDoc.fileName}\n\n`;
    reply += `File Size: **${(attachedDoc.fileSize / 1024).toFixed(1)} KB** | Format: **${attachedDoc.fileType.toUpperCase()}**\n\n`;
    reply += `I have received your document. Tell me how you'd like me to process it (e.g. *"Extract clients into directory"*, *"Create quotation from items"*, or *"Analyze sales trends"*).`;
  ...
  const reply = `I am **Binti**, your AI Operating Assistant for **${context?.companyName || "Binti Events Management System"}**.\n\n` +
    `You can ask me questions, or click the **` + `+` + `** button to upload business documents (spreadsheets, client CSVs, rate cards) so I can restructure and import them into your database.`;
